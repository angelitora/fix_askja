...
# No filename list to maintain — any image file (.png/.jpg/.jpeg/.svg/
# .gif) placed in docs/assets/logos/ shows up automatically, in
# alphabetical order. Rename with a numeric prefix (1_imo.png,
# 2_hi.png, ...) if you want to control the display order; otherwise
# it's whatever order the filenames sort in.

def get_logo_files():
    """Any image file dropped into docs/assets/logos/ is picked up
    automatically — no code change needed to add/remove/rename a logo."""
    if not os.path.isdir(LOGOS_DIR):
        return []
    exts = ("*.png", "*.jpg", "*.jpeg", "*.svg", "*.gif")
    files = []
    for ext in exts:
        files.extend(glob.glob(os.path.join(LOGOS_DIR, ext)))
...
def write_html(summary):
    ...
    # Auto-discovered logos — if none are uploaded yet, this is just an
    # empty list and the logo bar is skipped entirely (no broken links).
    logo_imgs = get_logo_files()
    logo_bar = ""
    if logo_imgs:
        imgs_html = "\n    ".join(
